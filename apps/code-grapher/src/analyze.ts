...
export function loadTSProgram(entryPath: string) {
  ...
  if (configJson.error) throw new Error('Invalid tsconfig.json');
...
export function analyze(entryPath: string) {
  const { program, checker } = loadTSProgram(entryPath);
  const sourceFile = program.getSourceFile(entryPath);
  if (!sourceFile) throw new Error('Could not read source file...');
   ...
   */
  function getFullyQualifiedName(inputSymbol: ts.Symbol): string {
  ...
  const symbolFullyQualifiedNameToNanoId = new Map<string, string>();
  const symbolIdToNanoId = new Map<string, string>();
  function getSymbolNanoId(symbol: ts.Symbol): string {
   ...
   */
  function getOriginalSymbol(symbol: ts.Symbol): ts.Symbol {
    try {
  ...
  const traversedSymbols = new WeakSet<ts.Symbol>();
  function traverse(inputSymbol: ts.Symbol) {
    const symbol = getOriginalSymbol(inputSymbol);
    if (traversedSymbols.has(symbol)) return;
    ...
    if (functionDeclaration) {
      ...
      const calls: Array<FunctionCall> = (functionDeclaration.body ? findCallsInNode(functionDeclaration.body) : []).map(callExpr => {
  ...
  for (const [_exportName, exportSymbol] of exports?.entries() ?? []) {
...
function getFunctionDeclaration(symbol: ts.Symbol): ts.FunctionDeclaration | undefined {
...
function findCallsInNode(node: ts.Node): ts.CallExpression[] {
...
function destructBindingPattern(
