...
function useAutoLayout(options: LayoutOptions) {
  ...
  useEffect(() => {
    ...
    if (!nodesInitialized || elements.nodes.length === 0) {
        console.log('return', nodesInitialized, elements.nodes.length);
      return;
    }
    ...
    const runLayout = async () => {
        if ((Date.now() - lastRun) < 500) {
          // If the layout was run less than 100ms ago, don't run it again.
          console.log('Skip runLayout');
            return;
        }
        lastRun = Date.now();
        console.log('Running layout algorithm', options.algorithm);
